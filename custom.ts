/**
 * aqee:
 * issue: need be init twice or only white screen shown.
 * todo:
 * rolling
 * 
  * RB-TFT1.8-V2 Block
  */
//% color="#275C6B" icon="\uf26c" weight=95 block="aqeeRB-TFT18-V2"
namespace RBTFT18 {
    // Display commands & constants
    export let TFTWIDTH = 162
    export let TFTHEIGHT = 130
    // let TFTWIDTH = 130
    // let TFTHEIGHT = 162
    ...
    //issue: need be called twice, or only white screen shown.
    /*
     * Initial TFT setup
     */
    //% block="Initialize TFT Display %va"
    //% weight=100
    export function init(va: number): void {
        // set SPI frequency
...
//Adafruit_ST7735_and_ST7789_Library\Adafruit_ST7735.cpp  Rcmd1[]
        ...
        send(TFTCommands.PWCTR2, [0xC5])// send(TFTCommands.PWCTR2, [0x8A, 0x2A])
...
//Adafruit_ST7735_and_ST7789_Library\Adafruit_ST7735.cpp  Bcmd[]
/*        // Software reset
        ...
        // Frame rate control - normal mode        // Set 16-bit color mode
        send(TFTCommands.COLMOD, [0x05])
        control.waitMicros(10000)

        send(TFTCommands.FRMCTR1, [0x00, 0x06, 0x03]) //[0x01, 0x2C, 0x2D]
        control.waitMicros(10000)
        // // Frame rate control - idle mode
        // send(TFTCommands.FRMCTR2, [0x01, 0x2C, 0x2D, 0x01, 0x2C, 0x2D])

        // Memory access control
        send(TFTCommands.MADCTL, [0x08]) //va*16+8; 0xC8 vertical; 0xA0 horizontal
        //control.waitMicros(100000)

        send(TFTCommands.DISSET5, [0x15, 0x02]) //va*16+8; 0xC8 vertical; 0xA0 horizontal
        //control.waitMicros(100000)

        // Display inversion control
        send(TFTCommands.INVCTR, [0x00]) //0x07

        // Display power control
        send(TFTCommands.PWCTR1, [0x02, 0x70])
        control.waitMicros(10000)
        send(TFTCommands.PWCTR2, [0x05])
        send(TFTCommands.PWCTR3, [0x01, 0x02])
        //send(TFTCommands.PWCTR4, [0x8A, 0x2A])
        //send(TFTCommands.PWCTR5, [0x8A, 0xEE])
        send(TFTCommands.VMCTR1, [0x3c, 0x38])//[0x0E]
        control.waitMicros(10000)
        send(TFTCommands.PWCTR6, [0x11, 0x15])
        ...
        // Set Gamma
        send(TFTCommands.GMCTRP1, [0x09, 0x16, 0x09, 0x20,       //     (Not entirely necessary, but provides
            0x21, 0x1B, 0x13, 0x19,       //      accurate colors)
            0x17, 0x15, 0x1E, 0x2B,
            0x04, 0x05, 0x02, 0x0E])
        send(TFTCommands.GMCTRN1, [0x0B, 0x14, 0x08, 0x1E,       //     (Not entirely necessary, but provides
        ...
        // Column address set
        send(TFTCommands.CASET, [0x00, 0x00, 0x00, TFTWIDTH - 1])
        control.waitMicros(100000)
        // Row address set
        send(TFTCommands.RASET, [0x00, 0x00, 0x00, TFTHEIGHT - 1])
        control.waitMicros(100000)
